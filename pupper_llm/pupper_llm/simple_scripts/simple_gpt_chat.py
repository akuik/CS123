...
class GPT4ConversationNode(Node):
    ...
    def get_gpt4_response(self, query):
        try:
            ...
            prompt = "You are a quadruped robotic dog. When I ask you to perform an action, you will describe on a high level what you should do to acheive this action. Respond in python code. You have access to pupper.walk(), pupper.turn_left(), pupper.turn_right(), pupper.bark(), and pupper.stop(). You should only output your answer in these calls without giving any natural language explanation."
